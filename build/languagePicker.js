...
function moveFocusToPickerTrigger(picker) {
    ...
    if (picker.trigger.getAttribute('aria-expanded') == 'false')
        return;
    if (((_a = document.activeElement) === null || _a === void 0 ? void 0 : _a.closest('.language-pick-dropdown')) == picker.dropdown)
...
function initListPicker(picker) {
    ...
    for (let i = 0; i < picker.options.length; i++) {
        let selected = picker.options[i].selected ? 'aria-selected="true"' : '', language = picker.options[i].getAttribute('lang');
...
function getLanguageUrl(_option) {
    return 'index.html';
}
